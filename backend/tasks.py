...
def execute_claim_analysis(claim: str, task_id: str = None, progress_callback = None) -> dict:
    """Core analysis logic executed either by Celery worker or by in-memory background thread."""
    def notify(step: str, progress: int):
    ...
    notify('Translating & Preprocessing...', 15)
    translation_meta = preprocess_and_translate_claim(claim)
    target_claim = translation_meta["processed_claim"]

    # Stage 2: Cache Scanning
    notify('Scanning Vector Cache...', 35)
    try:
        cache_check = search_similar_claim(target_claim)
        if cache_check.get("is_cached") and cache_check.get("score", 0) > 0.75:
        ...
        print(f"Vector cache check notice: {e}")

    # Stage 3: NER & Deep-Dive Forensic Analysis
    notify('Running Deep-Dive Forensics...', 60)
    try:
    ...
    verdict = "UNVERIFIED"
    confidence = 88.5
    explanation = "Forensic synthesis evaluation against public registers and threat telemetry."
    ...
    if client:
        try:
            ...
            print(f"OpenAI reasoning notice: {ai_err}")

    # Stage 4: Formatting and Persistent Archiving
    notify('Formatting Intelligence Dossiers...', 85)
    formatted_evidence = []
    ...
    try:
        ...
        print(f"Vector save notice: {e}")

    try:
        save_claim_to_graph(target_claim, verdict, extracted_entities)
    except Exception as e:
        print(f"Graph save notice: {e}")

    keywords_list = [ent if isinstance(ent, str) else ent.get("text", "") for ent in extracted_entities] if extracted_entities else ["Misinformation", "Threat"]
...
def run_claim_analysis_task(self, claim: str):
    """Background task delivering deep forensic breakdowns via Celery worker."""
    def celery_progress(step, progress):
        self.update_state(state='PROGRESS', meta={'step': step, 'progress': progress})

    try:
        return execute_claim_analysis(claim, task_id=self.request.id, progress_callback=celery_progress)
